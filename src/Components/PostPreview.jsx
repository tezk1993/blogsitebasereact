      ...
      {postImage ? 
        <div class="w-[64rem] m-4 h-[256px] bg-slate-200 rounded-xl shadow-xl flex flex-row">
            <div class="ml-4 mt-4 mb-4 mr-4 flex flex-col justify-between w-1/2  ">
              <button id='profile' class="flex flex-row gap-2 hover:bg-slate-50 p-2 rounded-lg transition-all duration-300">
                {authorImage ?
            ...
            <div class="w-1/2 mr-4 mt-2 mb-2 ">
              <h2 class="font-bold font-inter mb-1">{title.length > 35 ? title.substring(0,35) + "..." : title}</h2>
              <h3 class="italic font-inter mb-1">{subtitle}</h3>
              {summary !== undefined ? <div dangerouslySetInnerHTML={convertedSummary}/>: <div></div>}
            </div>
            <Link to={`/post/${id}`} class="m-auto text-3xl p-2 text-gray-400 transition-all duration-500 hover:text-gray-600">&#11166;</Link>
        </div> 
      :
      <div class="w-[64rem] m-4 h-[256px] bg-slate-200 rounded-xl shadow-xl flex flex-row">
