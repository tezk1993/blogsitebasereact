...
import { faCog, faRightFromBracket, faRightToBracket } from '@fortawesome/free-solid-svg-icons'
  ...
  return (
    <header class="flex items-center justify-between flex-row text-xl m-4 font-bold">
        <NavLink class="hover:underline m-4 md:m-6" to="/" >
            <img id='logo' referrerpolicy="no-referrer" src="gs://blogsite-adf9c.appspot.com/SiteImages/OpenBook_Logo.png" class="size-24" />
        </NavLink>
        ...
        {isAuth ? 
            ...
            <div class="group flex flex-row-reverse  items-center  transition-all rounded-md">
                ...
                <p class="text-lg invisible group-hover:visible mr-2 transition-all ">Create post</p>
            </div>
            <NavLink class="" to={`/profile/${userID}`}><FontAwesomeIcon icon={faUser}/></NavLink>
            <NavLink class="" to={`/profile/${userID}/settings`}><FontAwesomeIcon icon={faCog}/></NavLink>
        ...
        <h1 class="text-5xl font-lora absolute left-[50%] translate-x-[-50%] tracking-wider underline underline-offset-[16px]">OPEN BOOK</h1>
